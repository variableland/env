...
// Boot through the CommonJS entry (`server.cjs` -> `require("./src/server.ts")`),
// the same require()-based path the e2e webServer uses.
const SERVER_CJS = fileURLToPath(new URL("../../server.cjs", import.meta.url));
...
function runServer(env: Record<string, string>) {
  ...
  const result = spawnSync("deno", ["run", "--allow-net", "--allow-env", "--allow-read", SERVER_CJS], {
    cwd: CWD,
    env: { ...baseEnv, ...env },
    encoding: "utf8",
    timeout: 8_000,
