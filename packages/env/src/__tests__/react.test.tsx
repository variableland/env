...
import { CLIENT_ENV_SCRIPT_ID } from "../lib/index.ts";
import { ClientEnv } from "../react.tsx";

it("render the client env", async () => {
  const runtimeEnv = { ENV: "development" };

  const { container } = await render(<ClientEnv runtimeEnv={runtimeEnv} />);
...
it("renders as a script tag with type application/json", async () => {
  ...
  expect(el?.getAttribute("id")).toBe(CLIENT_ENV_SCRIPT_ID);
});
...
it("renders an empty object when runtimeEnv is empty", async () => {
  const { container } = await render(<ClientEnv runtimeEnv={{}} />);
...
it("updates content when runtimeEnv changes", async () => {
  ...
  const { container, rerender } = await render(<ClientEnv runtimeEnv={initial} />);

  const el = container.querySelector("#env");
  expect(JSON.parse(el?.textContent ?? "")).toStrictEqual(initial);

  await rerender(<ClientEnv runtimeEnv={updated} />);
