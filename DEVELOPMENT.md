...
The `package/` and `docs/` workspaces still expose plain `pnpm test` / `pnpm dev` / etc. inside their own `package.json` — those scripts are what the mise tasks ultimately delegate to. They're internal to each subproject; you only see them if you `cd` into the subproject and run `pnpm <script>` directly. That's fine — it's the right API at that scope.
