...
See the [SSR guide](/guides/ssr) for the full pattern with two files (`schema.public.ts` and `schema.server.ts`) and a flat env-var binding on the composed branch.
...
See [Env-var naming](/concepts/env-var-naming) for the full rules around `vars`.
