...
If you need each build artifact to contain **only** its env's config (others not present at all), use the [Vite plugin pattern](/guides/spa-vite-plugin) instead.
...
- All env configs are **shipped** in the deployment as separate chunks. The browser only downloads one, but the others exist as static assets. Use the [Vite plugin pattern](/guides/spa-vite-plugin) if that's not acceptable.
