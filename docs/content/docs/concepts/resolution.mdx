...
2. **`config`** — the loaded per-environment object (typically from `loadConfig` or the `#config` alias).
3. **Environment variable** — read from `process.env` on the server or `window.__env` (set by `<ClientEnv />`) in the browser.
...
By default `runtimeEnv` is `process.env` (server) or `window.__env` (browser, populated by `<ClientEnv />` or `readEnv()`). Override it when you need to:
